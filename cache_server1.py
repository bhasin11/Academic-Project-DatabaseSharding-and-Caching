...
from cache_database1 import Cache
from cache_database1 import db
from cache_database1 import createDatabase
...
def createEntry():
    ...
    try:
        ...
        id = int(ox['id'])
        newUser = Cache(id, ox['name'], ox['email'], ox['category'],ox['description'],
...
def withExpenseID(expense_id):
    ox2 = Cache.query.filter_by(id=expense_id).first_or_404()
...
if __name__ == '__main__':
    ...
    r_server.lrem("CACHE", "127.0.0.1:6001", 0)
    r_server.rpush("CACHE", "127.0.0.1:6001")
